...
빌드 없이 받으려면 [내려받기 페이지](https://agshwanbong-max.github.io/my-mac/).
...
공증까지 끝났으면 GitHub Release 로 올립니다.

```bash
./Scripts/publish.sh
```

`VERSION` 파일의 값으로 `v0.1.0` 태그를 만들고, 다음 세 개를 함께 올립니다.
...
같은 태그가 이미 있으면 멈춥니다. 공증 안 된 걸 올리면 받는 사람 맥에서 열리지 않습니다.

`appcast.json` 은 `releases/latest/download/appcast.json` 으로 **항상 최신 릴리스에서** 서빙되므로,
앱이 보는 주소는 한 번 정하면 바뀌지 않습니다.

새 버전을 낼 때는 `VERSION` 을 올리고 `release.sh` → `publish.sh` 순서로 다시 돌립니다.

### 다운로드 페이지

`docs/` 가 그대로 GitHub Pages 입니다 (Settings → Pages → Source: `main` / `docs`).

- `docs/index.html` — 소개 · 설치 안내 · 전체 디스크 접근 설명
- `docs/support.html` — 후원 페이지 (앱의 "후원하기" 메뉴가 여기로 옵니다)
...
조회는 `L("key")` 하나로 모았습니다. SwiftUI 의 `Text("…")` 는 **메인 번들**에서 찾는데
코어는 라이브러리라 자기 번들에 리소스가 들어가고, CLI 와 테스트에는 SwiftUI 자체가 없습니다.

**키를 못 찾으면 키 이름이 그대로 화면에 나옵니다.** 일부러 그렇게 뒀습니다 —
한국어로 조용히 되돌아가면 번역 누락을 아무도 발견하지 못합니다.
...
테스트가 사람이 못 지키는 것들을 대신 지킵니다.

- 모든 언어가 같은 키 집합을 갖는가 (빠진 키·남은 키 양쪽)
- **서식 지정자가 원본과 일치하는가.** 이게 제일 중요합니다 — `%d` 를 `%@` 로 잘못 옮기면
  `String(format:)` 이 정수를 포인터로 읽어서 **그 언어에서만 앱이 죽습니다.**
  한국어로 개발하는 동안에는 절대 재현되지 않습니다.
- 번역문에 한국어가 남아 있지 않은가 (한 글자만 남아도 눈으로는 안 보입니다)
...
하루에 한 번 `appcast.json` 을 읽어 새 버전이 있으면 목록 위에 띠로 알립니다.
시트로 가로막지 않습니다 — 하던 일을 끊을 만한 소식이 아닙니다.

**알리기만 하고 앱 안에서 받아 설치하지는 않습니다.** 그러려면 프레임워크(Sparkle)를
...
처음 열면 검사보다 안내가 먼저 나옵니다. 세 가지를 말합니다.

1. 이 앱이 무엇을 하는가
2. **왜 전체 디스크 접근이 필요한가** — macOS 는 이 권한 없이 `~/Library` 의 상당 부분을
   오류 없이 빈 폴더처럼 가립니다. 권한이 없으면 앱이 조용히 절반만 찾습니다.
3. 무엇을 하지 않는가

설정 앱에 다녀오면 창으로 돌아오는 순간 권한을 다시 확인합니다.
권한 없이도 시작할 수 있습니다 — 강제하면 앱을 닫아버립니다.
...
- 예약 검사 (LaunchAgent)
- 앱 안에서 업데이트를 받아 설치하기 (알림까지만 — 위 "업데이트 알림" 참고)
