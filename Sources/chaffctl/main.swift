...
//
//   chaffctl scan                    검사만 하고 표로 보여준다
//   chaffctl plan                    지울 항목을 골라 미리보기(dry run)
//   chaffctl clean --safe --confirm  안전 등급만 실제로 정리
//   chaffctl rules                   등록된 규칙 전부 출력
//   chaffctl log                     최근 작업 기록
...
case "plan", "clean":
    ...
    if dryRun {
        print("※ 미리보기 모드입니다. 실제로 지우려면: chaffctl clean --confirm\n")
    }
...
case "inspect":
    ...
    if targets.isEmpty {
        print("경로를 지정하세요. 예: chaffctl inspect ~/Downloads/something.dmg")
    }
