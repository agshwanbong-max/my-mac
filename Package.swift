...
let package = Package(
    ...
    platforms: [.macOS(.v13)],
    products: [
        .library(name: "ChaffCore", targets: ["ChaffCore"]),
        .executable(name: "chaffctl", targets: ["chaffctl"]),
        .executable(name: "Chaff", targets: ["ChaffApp"]),
    ],
    targets: [
        ...
        .executableTarget(name: "chaffctl", dependencies: ["ChaffCore"]),

        // SwiftUI 앱.
        .executableTarget(
            name: "ChaffApp",
            dependencies: ["ChaffCore"],
            // 아이콘은 `build_app.sh` 가 소스 경로에서 직접 번들로 복사한다.
            // SPM 리소스로 넘기면 `Chaff_ChaffApp.bundle` 안에 들어가는데,
            // `CFBundleIconFile` 은 Contents/Resources 바로 아래를 본다.
            exclude: ["Resources"]
        ),

        .testTarget(name: "ChaffCoreTests", dependencies: ["ChaffCore"]),
    ]
)
